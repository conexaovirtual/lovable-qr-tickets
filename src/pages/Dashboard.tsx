...
export default function Dashboard() {
    ...
    try {
      ...
      const results = await Promise.allSettled([
        ...
        supabase
          .from("service_orders")
          .select(
            "id, numero_os, data_agendada, hora_agendada, status, companies(nome_fantasia), profiles!service_orders_tecnico_id_fkey(nome)",
          )
          .gte("data_agendada", today.toISOString())
  ...
  const handleViewServiceOrder = async (osId: string) => {
    const { data, error } = await supabase
      .from("service_orders")
      .select(
        `*, tickets(numero, titulo), companies:company_id(nome_fantasia, cnpj, endereco), profiles:tecnico_id(nome)`,
      )
      .eq("id", osId)
      .single();
    if (!error && data) {
  ...
  const totalTickets =
    stats.tickets_novo + stats.tickets_em_atendimento + stats.tickets_resolvido + stats.tickets_fechado;
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        <span className="font-bold text-destructive">{stats.chamados_qrcode}</span> aguardando
                        atendimento
                      </p>
                    </div>
                  </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    {
                      label: "Novos",
                      val: stats.tickets_novo,
                      cls: "border-l-[hsl(var(--info))]",
                      textCls: "text-[hsl(var(--info))]",
                    },
                    {
                      label: "Em Atendimento",
                      val: stats.tickets_em_atendimento,
                      cls: "border-l-[hsl(var(--warning))]",
                      textCls: "text-[hsl(var(--warning))]",
                    },
                    {
                      label: "Resolvidos",
                      val: stats.tickets_resolvido,
                      cls: "border-l-[hsl(var(--success))]",
                      textCls: "text-[hsl(var(--success))]",
                    },
                    { label: "Fechados", val: stats.tickets_fechado, cls: "border-l-muted-foreground", textCls: "" },
                  ].map(({ label, val, cls, textCls }) => (
                    <Card
                      key={label}
                      className={`cursor-pointer hover:shadow-md transition-all border-l-4 ${cls}`}
                      onClick={() => navigate("/tickets")}
                    >
              ...
              <Card>
                ...
                <CardContent>
                  {recentServices.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-6">Nenhum atendimento recente</p>
                  ) : (
                    <div className="space-y-2">
            ...
            <Card>
              ...
              <CardContent className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
                ...
                <QuickAction
                  icon={MessageSquare}
                  label="WhatsApp"
                  onClick={() => navigate("/whatsapp-platform")}
                  variant="primary"
                />
              </CardContent>
            </Card>
  ...
  const colorClasses =
    variant === "purple"
      ...
      : variant === "primary"
        ...
        : "hover:bg-accent";
  return (
    <Button variant="outline" className={`h-auto flex-col gap-1.5 py-3 ${colorClasses}`} onClick={onClick}>
