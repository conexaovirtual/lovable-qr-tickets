...
export function SmartAlertsPanel() {
  const [alerts, setAlerts] = useState<Alert[]>(_cachedAlerts);
  const [loading, setLoading] = useState(_cachedAlerts.length === 0);
  const [generating, setGenerating] = useState(false);
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null);

  const autoGeneratedRef = useRef(false);

  const loadAlerts = useCallback(async (showSkeleton = false) => {
    ...
    try {
      ...
      _cachedAlerts = loaded;
      setAlerts(loaded);

      // Gera alertas automaticamente apenas uma vez por montagem do componente
  ...
  useEffect(() => {
    loadAlerts(_cachedAlerts.length === 0); // skeleton só se cache vazio
  ...
  const generateAlerts = async (silent = false) => {
    if (!silent) setGenerating(true);
    try {
      const { data, error } = await supabase.functions.invoke('ai-smart-alerts', { body: {} });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      if (!silent) toast.success(`${data.alertas?.length || 0} alertas analisados`);
      loadAlerts();
    } catch (err: any) {
      if (!silent) toast.error('Erro ao gerar alertas');
    } finally {
      if (!silent) setGenerating(false);
    }
  };

  const handleGenerateAlerts = () => generateAlerts(false);
